			...
			<div class="main-panel">
				<div class="content-wrapper">
								...
								<div class="card-body">
									...
									<div class="form-group"
										...
										<select id="_searchType" name="_searchType"
											...
											<option value="1"
												<c:if test = '${searchType eq "1"}'>selected</c:if>>상품명</option>
										</select>
										...
										<div style="margin-right: 20px; margin-top: 10px">
											<input type="button" onClick="fn_upload(${curPage})"
												class="btn btn-primary" value="상품 올리기">
										</div>
									</div>
											...
											<c:if test="${!empty list}">
												<c:forEach var="product" items="${list}" varStatus="status">
													<tr>
														...
														<td><img
															src="/resources/upload/product/${product.productSeq}.png"></td>
														...
														<td><c:set var="originalDate"
																value="${product.productStartDate}" /> <c:set
																var="dateParts" value="${fn:split(originalDate, ' ')}" />
															<c:set var="dateOnly" value="${dateParts[0]}" />
															${dateOnly}</td>
														<td><c:set var="originalDate1"
																value="${product.productEndDate}" /> <c:set
																var="dateParts1" value="${fn:split(originalDate1, ' ')}" />
															<c:set var="dateOnly1" value="${dateParts1[0]}" /> <span>${dateOnly1}</span></td>
														<td><label class="badge badge-danger"
															style="cursor: pointer;"
															onclick="fn_update(${product.productSeq})">수정</label>
															&nbsp; <label class="badge badge-warning"
															style="cursor: pointer;"
															onclick="fn_delete(${product.productSeq})">삭제</label></td>
													</tr>
					...
					<form name="bbsForm" id="bbsForm" method="post">
						<input type="hidden" name="productSeq" value="" /> <input
