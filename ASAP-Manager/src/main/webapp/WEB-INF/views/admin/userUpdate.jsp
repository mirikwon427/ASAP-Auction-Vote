										...
										<form class="forms-sample">
											<div class="form-group">
												...
												<div class="row form-group"
													style="margin-bottom: 10px; margin-left: 0.5px;'">
													<input type="number" class="form-control"
														name="userChargePlus" id="userChargePlus"
														style="width: 10rem; height: 2.5rem; margin-right: 10px;">
													<button type="button" id="btnChargePlus"
														class="btn btn-primary">포인트 지급</button>
												</div>
											</div>
											<div class="form-group">
												<label for="exampleInputEmail3">포인트 *</label> <input
													type="text" class="form-control" name="userCharge"
													id="userCharge" value="${user.userCharge}" readonly>
											</div>
											<div class="form-group">
												<label for="exampleInputPassword4">닉네임 *</label> <input
													type="text" class="form-control" name="userNick"
													id="userNick" value="${user.userNick}" readonly>
											</div>
											<div class="form-group">
												<label for="exampleTextarea1">이름 *</label> <input
													type="text" class="form-control" name="userName"
													id="userName" value="${user.userName}" readonly>
											</div>
											<div class="form-group">
												<label for="exampleInputCity1">비밀번호 *</label> <input
													type="password" class="form-control" name="userPwd1"
													id="userPwd1" value="${user.userPwd}">
											</div>
											<div class="form-group">
												<label for="exampleTextarea1">비밀번호 확인 *</label> <input
													type="password" class="form-control" name="userPwd2"
													id="userPwd2" value="${user.userPwd}">
											</div>
											<div class="form-group">
												<label for="exampleTextarea1">이메일 *</label> <input
													type="text" class="form-control" name="userEmail"
													id="userEmail" value="${user.userEmail}">
											</div>
											<div class="form-group">
												<label for="exampleTextarea1">전화번호 *</label> <input
													type="text" class="form-control" name="userPhone"
													id="userPhone" value="${user.userPhone}">
											</div>
											<div class="form-group">
												<label for="exampleSelectGender">회원상태 *</label> <select
													class="form-control" name="userStatus" id="userStatus"
													style="font-size: 0.8rem; width: 8rem; height: 2.5rem;">
													<option value="">회원상태</option>
													<option value="Y"
														<c:if test="${user.status == 'Y'}">selected</c:if>>정상</option>
													<option value="N"
														<c:if test="${user.status == 'N'}">selected</c:if>>정지</option>
												</select>
											</div>
											<div class="form-group">
												...
												<div class="row form-group"
													style="margin-bottom: 10px; margin-left: 0.5px;'">
													<input type="text" class="form-control" name="userPostcode"
														id="userPostcode" value="${user.userPostcode}"
														style="width: 8rem; height: 2.5rem; margin-right: 10px;">
													<button type="button" onclick="execDaumPostcode()"
														class="btn btn-primary">우편번호 찾기</button>

												</div>
												<input type="text" class="form-control" name="userAddr"
													id="userAddr" value="${user.userAddr}"> <span
													id="guide" style="color: #999; display: none"></span>
											</div>
											<input type="button" class="btn btn-light" name="cancel"
												id="cancel" value="  취소  "> <input type="button"
												name="btnUpdate" id="btnUpdate" class="btn btn-primary ml-1"
												value="  수정  "> <input type="hidden" name="userPwd"
												id="userPwd" value=""> <input type="hidden"
												name="userId2" id="userId2" value="${user.userId}">
											<input type="hidden" name="userIdSearch" id="userIdSearch"
												value="${userIdSearch}"> <input type="hidden"
												name="curPage" id="curPage" value="${curPage}"> <input
												type="hidden" name="userCode" id="userCode"
												value="${userCode}"> <input type="hidden"
