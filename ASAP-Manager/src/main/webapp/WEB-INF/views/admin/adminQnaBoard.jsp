			...
			<div class="main-panel">
				<div class="content-wrapper">
									...
									<div class="form-group"
										...
										<div style="margin-right: 20px; margin-top: 10px">
											<button type="button" class="btn btn-inverse-danger btn-fw"
												onclick="showResult(1)">답변대기</button>
											<button type="button" class="btn btn-inverse-primary btn-fw"
												onclick="showResult(2)">답변완료</button>
										</div>
									</div>
											...
											<tbody>
												<c:if test="${!empty list}">
													<c:forEach var="qnaBoard" items="${list}"
														varStatus="status">
														<input type="hidden" name="status" id="stauts"
															value="${qnaBoard.status}" />
														<tr>
															<c:if test="${qnaBoard.qaIndent eq 0}">
																...
																<td><c:choose>
																		...
																		<c:otherwise>
																			<a id="view" href="javascript:void(0)"
																				onclick="fn_view(${qnaBoard.qaSeq})">
																				${qnaBoard.qaTitle} </a>
																		</c:otherwise>
																	</c:choose></td>
																...
																<td><label class="badge badge-warning"
																	...
																	onclick="fn_reply(${qnaBoard.qaSeq})">답변</label> &nbsp;
																	<label class="badge badge-danger"
																	style="cursor: pointer;"
																	onclick="fn_delete(${qnaBoard.qaSeq})">삭제</label></td>
															</c:if>
														</tr>
													</c:forEach>
												</c:if>
										...
										<table class="table table-striped">
											...
											<tbody>
												<c:if test="${!empty list2}">
													<c:forEach var="qnaBoard" items="${list2}"
														varStatus="status">
														<input type="hidden" name="status2" id="stauts2"
															value="${qnaBoard.status}" />
														<tr>
															<td>${qnaBoard.qaSeq}</td>
															<td>${qnaBoard.userId}</td>
					...
					<form name="bbsForm" id="bbsForm" method="post">
						<input type="hidden" name="qaSeq" id="qaSeq" value="" /> <input
							type="hidden" name="searchType" value="${searchType}" />
						<!-- modelMapd의 변수 -->
						<input type="hidden" name="searchValue" value="${searchValue}" />
						<input type="hidden" name="curPage" id="curPage"
							value="${curPage}" /> <input type="hidden" name="curPage2"
							id="curPage2" value="${curPage2}" /> <input type="hidden"
							name="resultNumber" id="resultNumber" value="" />
					</form>
				</div>
				<!-- main-panel ends -->
			</div>
