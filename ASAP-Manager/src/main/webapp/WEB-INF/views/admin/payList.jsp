			...
			<div class="main-panel">
				<div class="content-wrapper">
								...
								<div class="card-body">
									...
									<div class="form-group"
										...
										<select id="_searchType" name="_searchType"
											...
											<option value="1"
												<c:if test = '${searchType eq "1"}'>selected</c:if>>주문번호</option>
											<option value="2"
												<c:if test = '${searchType eq "2"}'>selected</c:if>>결제일</option>
											<option value="3"
												<c:if test = '${searchType eq "3"}'>selected</c:if>>회원아이디</option>
											<option value="4"
												<c:if test = '${searchType eq "4"}'>selected</c:if>>상품명</option>
											<option value="5"
												<c:if test = '${searchType eq "5"}'>selected</c:if>>결제상태</option>
										</select>
										...
										<div class="total"
											style="margin-top: 15px !important; text-align: right !important;">
												...
												<c:forEach var="pay" items="${list}" varStatus="status">
													<tr>
														...
														<td><c:set var="originalDate" value="${pay.regDate}" />
															<c:set var="dateParts"
																value="${fn:split(originalDate, ' ')}" /> <c:set
																var="dateOnly" value="${dateParts[0]}" /> ${dateOnly}</td>
														<td>${pay.userId}</td>
														<c:choose>
															<c:when test="${pay.cnt >1 }">
																<td>
																	<div class="cell-inner"
																		...
																		onclick="fn_view(${pay.orderNo})">
																		${pay.productName} 외 ${pay.cnt -1 }개</div>
																</td>
															</c:when>
															<c:otherwise>
																<td>
														...
														<td><fmt:formatNumber type="number"
																maxFractionDigits="3" value="${pay.payTotalPrice}" /></td>
														<td><fmt:formatNumber type="number"
																maxFractionDigits="3" value="${pay.payPointPrice}" /></td>
														<td><fmt:formatNumber type="number"
																maxFractionDigits="3" value="${pay.payRealPrice}" /></td>
														<td>${pay.status}</td>
													</tr>
					...
					<form name="bbsForm" id="bbsForm" method="post">
						<input type="hidden" name="orderNo" value="" /> <input
